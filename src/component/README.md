...
- **Fields:** `state: S` (public getter; assignment after construction warns), `props: P` — available in subclasses and externally.
...
- Direct `this.state = …` after init is forbidden: it emits **`console.warn`** (value is still written to the backing store, but **without** `onUpdate` / reconcile). Prefer `setState`.
