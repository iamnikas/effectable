 ...
 * root* type. Nested `@OnCommand` / `@OnQuery` under a PLACE wrapper, or under
 * a deferred UPDATE's nested PLACE, still collided with a leaving orphan → fail-stop.
 *
 * Distinct from exclusive release under a *surviving* later UPDATE sibling, and from
 * sibling-root PLACE vs orphan (control below).
 */
...
describe('GraphRuntime nested PLACE vs orphan exclusive', () => {
  it('CONTROL: sibling-root PLACE vs keyed orphan @OnCommand', async () => {
  ...
  it('UPDATE onUpdate execute still reaches orphan when no nested PLACE claims DO', async () => {
    const seen: string[] = [];
