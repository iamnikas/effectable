 ...
 *
 * Covers initial mount, PLACE batches on reconcile, and REPLACE+PLACE batches
 * (type/key change must defer onMount the same way as PLACE).
 *
...
describe('GraphRuntime sibling bus mount order', () => {
  ...
  it('REPLACE publisher + PLACE listener: replaced onMount publish reaches new sibling @OnEvent', async () => {
    const seen: string[] = [];
    const buses = makeBuses();
    ...
    class Publisher extends Component {
      @UseEventBus() declare events: EventBus<Ev>;
      public override onMount (): void {
        this.events.publish({ type: 'READY', payload: { id: 'replaced' } });
      }
    }
    ...
    class Parent extends Component<{ mode: 'init' | 'expand' }, { mode: 'init' | 'expand' }> {
      public override compose () {
        if (this.props.mode === 'init') {
          return [h(Placeholder, {}, 'a')];
        }
        // Key 'a' REPLACE Placeholder→Publisher; key 'b' PLACE Listener.
        // Without REPLACE deferral, Publisher.onMount runs before Listener buses wire.
        return [h(Publisher, {}, 'a'), h(Listener, {}, 'b')];
      }
    }
    ...
    await rt.reconcile(h(Parent, { mode: 'expand' }));
    expect(rt.isActive()).toBe(true);
